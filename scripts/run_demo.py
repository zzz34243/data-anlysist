...
if __name__ == "__main__":
    ...
    result = Orchestrator(memory, settings.storage_path, max_filter_rows=settings.max_filter_rows, max_retries=settings.max_retries).analyze(project_name="示例销售项目", request="分析产品和渠道趋势，制定下一周期营销策略", csv_path=str(Path(__file__).resolve().parents[1] / "data" / "sample_sales.csv"))
