...
class Orchestrator:
    ...
    def analyze(self, *, project_name: str, request: str, data: list[dict[str, Any]] | None = None, csv_path: str | None = None, source_name: str | None = None, project_id: str | None = None, product_mode: str = "offline", requested_types: list[str] | None = None, chart_types: list[str] | None = None, filters: dict[str, Any] | None = None, date_range: str | None = None, privacy_consent: bool = True) -> dict[str, Any]:
        ...
        context = {"run_id": run_id, "project_id": project_id, "project_name": project_name, "request": request, "storage_path": str(self.storage_path), "selection": optimized["selection"], "optimized_prompt": optimized["optimized_prompt"], "rag_context": self.rag.build_context(request, project_id=project_id), "max_filter_rows": self.max_filter_rows}
        steps = devote()
        ...
        try:
            ...
            prepared = self._step(run_id=run_id, order=1, agent_name="data_preparation", payload={"data": data, "csv_path": csv_path, "source_name": source_name, "filters": filters}, context=context, fn=lambda: self.run_agent("data_preparation", {"data": data, "csv_path": csv_path, "source_name": source_name, "filters": filters}, context))
            ...
            validation = self._step(run_id=run_id, order=4, agent_name="validator", payload={"prepared_data": True, "insights": True}, context=context, fn=lambda: self.run_agent("validator", {"rows": prepared["rows"], "summary": prepared["summary"], "aggregates": prepared["aggregates"], "insights": insights}, context)); self.memory.update_todo(actor="validator", todo_id=todo_ids["validator4"], status="completed", result=validation)
            strategy = self._step(run_id=run_id, order=5, agent_name="marketing", payload={"insights": True, "validation": True, "summary": True}, context=context, fn=lambda: self.run_agent("marketing", {"insights": insights, "validation": validation, "summary": prepared["summary"]}, context)); self.memory.update_todo(actor="marketing", todo_id=todo_ids["marketing5"], status="completed", result=strategy)
            report = self._step(run_id=run_id, order=6, agent_name="report", payload={"prepared_data": True, "charts": True, "insights": True, "validation": True, "strategy": True}, context=context, fn=lambda: self.run_agent("report", {"prepared": prepared, "charts": charts, "insights": insights, "validation": validation, "strategy": strategy}, context)); self.memory.update_todo(actor="report", todo_id=todo_ids["report6"], status="completed", result={"path": report["path"], "pdf_path": report["pdf_path"]})
            final_validation = self._step(run_id=run_id, order=7, agent_name="validator", payload={"report": True}, context=context, fn=lambda: self.run_agent("validator", {"rows": prepared["rows"], "summary": prepared["summary"], "aggregates": prepared["aggregates"], "insights": insights, "report": report}, context)); self.memory.update_todo(actor="validator", todo_id=todo_ids["validator7"], status="completed", result=final_validation)
            snapshot = {"run_id": run_id, "project_id": project_id, "plan_id": plan_id, "request": run_request, "optimized": optimized, "privacy": privacy, "rag_context": context["rag_context"], "dataset_id": dataset_id, "prepared_summary": prepared["summary"], "charts": charts, "insights": insights, "validation": validation, "strategy": strategy, "report_validation": final_validation, "report": {"path": report["path"], "pdf_path": report["pdf_path"]}, "finished_at": utc_now()}
